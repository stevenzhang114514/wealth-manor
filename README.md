...
> 工行APP游戏化智能理财与个人财产管理平台 —— 可运行演示原型（成品版）
> "让理财像玩游戏一样简单，让每个人都能看清自己的全部财富版图。"

本项目是《工行理财游戏化设计方案》的可运行原型，以 **375px 手机框**模拟工行APP内嵌 H5 形态。覆盖**全量核心玩法**：新手引导+庄园创建、庄园主页（种植/天气/收获）、资产看板（配置/趋势/AI建议/财富树）、任务中心、知识答题、装扮商城、好友排行、AI助手对话、资产导入、目标规划。前后端分离，接口遵循 RESTful 规范，数据层可扩展（Mock ⇄ 真实工行接口）。
...
- 底部 Tab 切换四个主模块：🏡庄园 / 📊资产 / 📋任务 / 🎓答题；商城/好友/AI助手等从庄园与资产页内入口进入
- 桌面浏览器两侧为产品说明面板；`http://localhost:5173/manor?embed=1` 为整屏模式（截图/投屏用）
- 后端健康检查：http://localhost:3000/healthz

## 演示路径（对应企划书 3.5 平台演示）

1. **新手引导**：首次进入自动触发 6 步引导 → 创建庄园（名称+风格）；庄园页「新手引导」可重玩
