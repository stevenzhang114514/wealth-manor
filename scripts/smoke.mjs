...
const run = async () => {
  ...
  console.log('▶ AI 翻译器与体检（合规版）')
  r = await post('/ai/chat', { message: '什么是ETF？' })
  check('翻译器词条命中', r.body.data?.type === 'glossary' && r.body.data?.term === 'ETF')
  check('翻译器含免责声明', r.body.data?.disclaimer?.includes('不构成任何投资建议'))
  check('翻译器零建议措辞', !r.body.data?.explain?.includes('建议') && !r.body.data?.explain?.includes('买入'))
  r = await post('/ai/chat', { message: '' })
  check('空消息 40001', r.body.code === 40001)
  r = await get('/ai/portfolio-report')
  check('体检报告只陈述事实', Array.isArray(r.body.data?.facts) && Array.isArray(r.body.data?.riskNotes) && !('optimizePlan' in (r.body.data ?? {})))
  ...
  console.log('▶ 夺金冒险（开箱）')
  r = await get('/adventure/difficulties')
  check('四难度配置', r.body.data?.length === 4)
  r = await get('/adventure/containers?level=R1')
  check('容器风评门槛（R1仅债券）', r.body.data?.find((c) => c.id === 'stock')?.lock?.type === 'risk')
  ...
  check('开局 1000 金币·双容器', r.body.data?.gold === 1000 && r.body.data?.containers?.length === 2)
  const rid = r.body.data.id
  r = await post(`/adventure/run/${rid}/extract`)
  check('未达标撤离 409', r.status === 409)
  r = await post(`/adventure/run/${rid}/step`, { containerId: 'fund' })
  check('开箱一步（品质/板块/容器）', r.body.data?.turn === 1 && r.body.data?.lastStep?.quality?.id && r.body.data?.lastStep?.sector?.id && r.body.data?.lastStep?.container?.id === 'fund')
  r = await get('/adventure/rank')
  check('段位榜含本人', r.body.data?.self?.tier && r.body.data?.list?.length >= 6)

  console.log('▶ 偷菜')
  r = await post('/social/steal/f_002')
  check('偷菜成功 5~30 金币', r.body.data?.rewards?.coins >= 5 && r.body.data?.rewards?.coins <= 30)
  r = await post('/social/steal/f_002')
  check('重复偷菜 409', r.status === 409)
