...
const run = async () => {
  ...
  r = await get('/adventure/gear?level=R3')
  check('装备风评门槛（R4产品锁定）', r.body.data?.find((p) => p.id === 'p_stock')?.lock?.type === 'risk')
  r = await post('/adventure/run', { difficultyId: 'easy', gear: ['p_mmf', 'p_shortbond'], riskLevel: 'R3' })
  check('开局 1000 金币', r.body.data?.gold === 1000 && r.body.data?.status === 'playing')
  const rid = r.body.data.id
  r = await post(`/adventure/run/${rid}/extract`)
  check('未达标撤离 409', r.status === 409)
  r = await post(`/adventure/run/${rid}/step`)
  check('摸金一步推进', r.body.data?.turn === 1 && r.body.data?.lastStep)
