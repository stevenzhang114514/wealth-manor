...
可替换的Mock数据层）。原型为\textbf{独立产品形态}的「个人理财系统」，
采用\textbf{Apple 设计语言}（iOS 蓝色系色板、毛玻璃卡片、连续圆角体系、
分段控件）并适配多端分辨率——手机全屏（安全区适配）、平板居中宽屏、
桌面 375px 手机框按视口高度自动缩放。覆盖登录、新手引导、庄园主页、
资产看板、任务中心、知识答题、装扮商城、好友排行、AI助手对话、
目标规划、资产导入与个人中心\textbf{全量核心模块}，全部数据由后端接口
...
\midrule
Step 1 & 登录 & 手机号一键登录（演示模式任意11位手机号）；
        登录守卫保护全站页面 \\
Step 2 & 新手引导 & 首次进入自动触发6步引导；选择中式/西式/日式风格，
        创建专属庄园，领取新手礼包 \\
Step 3 & 庄园主页 & 天气=今日行情（晴/雨/彩虹特效）；植物分五阶段生长，
        点击植物查看关联理财产品的持仓与收益；成熟后"收获"丰收奖励入账 \\
Step 4 & 资产看板 & 总净资产一屏总览；饼图6类资产配置；30天净资产趋势；
        健康度评分；AI建议卡触发风险预警与"一键优化"；财富树可视化 \\
Step 5 & 任务中心 & 日常/周/月/成就四类任务；进度条与奖励；点击领取，
        金币经验实时入账（金币飞入动画） \\
Step 6 & 知识答题 & 每日5题理财知识；提交即时判分；得分兑换金币/经验，
        满分获徽章碎片 \\
Step 7 & 商城与社交 & 装扮商城金币/钻石兑换，装备后庄园实景渲染；
        好友互访浇水、月度配置合理性PK排行榜 \\
Step 8 & AI助手与规划 & AI助手规则引擎问答；购房/教育/养老/应急
        目标测算（4\%年化复利）与资产四通道导入 \\
Step 9 & 个人中心 & 头像/风险等级、庄园与资产概览、目标入口、退出登录 \\
\bottomrule
\end{tabular}
\caption{平台演示路径}
\end{table}
...
\IfFileExists{img/manor.png}{
  \begin{figure}[H]
  \centering
  \includegraphics[width=0.23\textwidth]{img/login.png}\hfill
  ...
  \includegraphics[width=0.23\textwidth]{img/profile.png}
  \caption{个人理财系统演示原型（上排左起：登录、庄园主页、资产看板、任务中心；
          下排左起：知识答题、装扮商城、好友与排行、个人中心）}
  \end{figure}
}{
  \begin{center}
