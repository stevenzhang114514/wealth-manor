...
\subsubsection{1. 模块一：夺金冒险局（核心玩法）}

\textbf{（1）板块建筑随机游走。}摸金的地图由四类\textbf{板块建筑}随机生成并连在一起——\textbf{经济作物}（小麦/棉花/咖啡豆）、\textbf{粮油}（大豆/玉米/菜籽油）、\textbf{金属}（铜/铝/锂/稀土）、\textbf{油气}（原油/天然气/石化品），每一步都可能走进不同板块。每个板块拥有受\textbf{经济周期加成与随机波动}共同决定的景气度（景气🔥/平稳/低迷❄️），以及专属的掉落物目录——"进哪个建筑"本身就有运气成分。

\textbf{（2）容器 = 投资方式。}建筑中可能装有物品的容器，分别对应三种投资方式，开局\textbf{三选二}携带、每步开箱前可切换：
...
\bottomrule
\end{tabular}
\caption*{表3-6 \quad 容器（投资方式）三卡配置}
\end{table}

\textbf{（3）开容器 = 随机事件：五档品质掉落。}开箱即按\textbf{品质掉落表}随机产出物品——\textbf{更高品质更值钱，但概率更低}：

\begin{table}[htbp]
\centering
\begin{tabular}{p{2.2cm}p{2cm}p{2.4cm}p{2.6cm}}
\toprule
\textbf{品质} & \textbf{标识} & \textbf{价值倍数} & \textbf{掉落概率} \\
\midrule
普通 & ⚪ 白 & $\times$1 & 70\% \\
良品 & 🟢 绿 & $\times$2 & 20\% \\
稀有 & 🔵 蓝 & $\times$4 & 8\% \\
...
\bottomrule
\end{tabular}
\caption*{表3-7 \quad 开箱品质掉落表}
\end{table}

\textbf{（4）价值公式与随机修饰。}单次开箱价值 = 掉落物基础值 $\times$ 品质倍数 $\times$（1+板块景气度）$\times$ 容器波动抽样；在此基础上叠加\textbf{随机暴击}（收益$\times$1.5$\sim$3，肾上腺素飙升）与\textbf{市场黑天鹅}（大幅回撤）；此外还有\textbf{双副收益}：其一为金币小奖（按容器概率额外+5$\sim$50金币，如"摸到金豆边角料"），其二为品质升级（按容器概率将掉落物品质直接升一档，如普通$\rightarrow$良品）。收益率不预设——景气、品质、波动全部随机，收益是分布的。

\textbf{（5）撤离、排位与消费闭环。}每局达到目标数额（初始资金的1.5$\sim$5倍，按四难度）即可\textbf{撤离并带出收益}；金币清零或超时未达标则血本无归。收益以游戏金币计，按\textbf{100金币 = 1元}换算为现实收入展示；成功撤离获得排位分，累计排位分映射\textbf{青铜—白银—黄金—铂金—钻石—王者}六段位。撤离收益进入庄园金币账户，用于\textbf{庄园装饰升级与购买家具}；好友庄园\textbf{偷菜}（每日限1次随机金币）、浇水互访与段位榜构成社交闭环。

\textbf{（6）3分钟新手留存设计。}登录即开局：首次进入自动开始简单模式新手局（前3回合强制无黑天鹅），"进板块—开箱子—攒够—撤离"四步即懂、30秒内获得第一次品质掉落的正反馈；风险评估不再是开局门槛，改为可选的"装备解锁评估"（完成可解锁更高风险容器）。
...
\bottomrule
\end{tabular}
\caption*{表3-9 \quad 资产体检与测算功能（合规口径）}
\end{table}
