...
\textbf{（1）运气 $\times$ 策略的摸金撤离循环。}游戏第一原动力是即时乐趣：每局以1000金币开局，随机事件驱动经营——摸金机会（牛市行情、低位建仓、政策红包）、市场黑天鹅（股灾、暴雷、流动性冻结）与中性事件交替出现。与固定收益率设定不同，装备产品的收益来自\textbf{经济周期随机切换 + 产品历史收益分布随机抽样}的叠加（如指数基金按历史均值与波动率正态抽样），并叠加\textbf{随机暴击}（收益$\times$1.5$\sim$3）与\textbf{黑天鹅暴跌}——每一步都可能肾上腺素飙升。玩家的核心决策只有两个按钮：\textbf{继续摸金 or 撤离}——贪心与纪律的博弈本身就是财富管理的隐喻。

\textbf{（2）撤离与收益闭环。}每局达到目标数额（初始资金的1.5$\sim$5倍）即可\textbf{撤离并带出收益}；金币清零或超时未达标则血本无归。收益以游戏金币计，按\textbf{100金币 = 1元}换算为现实收入展示，让"冒险所得"与"现实财富"同构。

\textbf{（3）四难度与排位段位。}简单/普通/困难/噩梦四种模式（黑天鹅概率5\%$\sim$30\%、暴击倍率$\times$2$\sim$$\times$3、排位系数1$\sim$3），成功撤离获得排位分，累计排位分映射\textbf{青铜—白银—黄金—铂金—钻石—王者}六段位。

\begin{table}[htbp]
\centering
\begin{tabular}{p{2cm}p{2.6cm}p{2.2cm}p{2.4cm}p{2.4cm}}
\toprule
\textbf{模式} & \textbf{目标} & \textbf{黑天鹅} & \textbf{暴击} & \textbf{排位系数} \\
\midrule
简单 & 1.5倍 & 5\% & $\times$2 & $\times$1 \\
普通 & 2倍 & 12\% & $\times$2 & $\times$1.5 \\
困难 & 3倍 & 20\% & $\times$2.5 & $\times$2 \\
噩梦 & 5倍 & 30\% & $\times$3 & $\times$3 \\
\bottomrule
\end{tabular}
\caption*{表3-6 \quad 夺金冒险四难度配置}
\end{table}

\textbf{（4）社交与消费闭环。}撤离收益直接进入庄园金币账户，用于\textbf{庄园装饰升级与购买家具}；好友庄园新增\textbf{偷菜}玩法（每日限1次，随机金币入账），配合浇水互访与\textbf{段位榜}PK，形成"冒险赚币—庄园消费—好友攀比"的社交闭环。

\textbf{（5）3分钟新手留存设计。}登录即开局：首次进入自动开始简单模式新手局（前3回合强制无黑天鹅），"摸金—攒够—撤离"三步即懂、30秒内获得第一次正反馈；风险评估不再是开局门槛，改为可选的"装备解锁评估"（完成可解锁更高风险产品卡）。
...
\bottomrule
\end{tabular}
\caption*{表3-8 \quad 资产体检与测算功能（合规口径）}
\end{table}
...
\textbf{首次使用流程}：手机号登录 $\rightarrow$ 新手局自动开启（简单模式摸金）$\rightarrow$ 攒够目标撤离获第一笔收益 $\rightarrow$ 用收益装扮庄园 $\rightarrow$ 尝试更高难度冲击段位 $\rightarrow$（可选）装备解锁评估与生涯模式深度学习。
...
\subsubsection{六大创新点}
\begin{enumerate}
  \item \textbf{摸金撤离——运气 $\times$ 策略的肾上腺素循环}：以"夺金撤离"取代说教式经营，每一步"继续摸金 or 撤离"的博弈把延迟满足的财富管理哲学装进即时乐趣的游戏外壳；
...
Step 2 & 夺金冒险开局 & 首次进入自动开启简单模式新手局（前3回合无黑天鹅），三步即懂摸金节奏 \\
Step 3 & 摸金决策 & 随机事件卡（摸金机会/黑天鹅/中性）；收益按历史分布抽样，暴击$\times$2$\sim$$\times$3与黑天鹅特效；「继续摸金 or 撤离」双按钮博弈 \\
